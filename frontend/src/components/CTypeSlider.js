...
const CTypeSlider = ({ viewAll }) => {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const response = await axios.get('http://ec2-16-170-217-216.eu-north-1.compute.amazonaws.com:5000/api/course-category/categories');
        ...
        await Promise.all(fetchedCategories.map(async (category) => {
          try {
            const feeResponse = await axios.get(`http://ec2-16-170-217-216.eu-north-1.compute.amazonaws.com:5000/api/courses/lowest-fee/${category.name}`);
