...
const ActivityInfo = () => {
    ...
    useEffect(() => {
        ...
        const fetchCourseData = async () => {
            try {
                const courseResponse = await axios.get(`http://ec2-16-170-217-216.eu-north-1.compute.amazonaws.com:5000/api/courses/course/${courseId}`);
                setCourse(courseResponse.data);

                if (courseResponse.data && courseResponse.data.providerId) {
                    const providerResponse = await axios.get(`http://ec2-16-170-217-216.eu-north-1.compute.amazonaws.com:5000/api/users/provider/${courseResponse.data.providerId}`);
